...
G920_HOT uint8_t g920_frame_priority(g920_frame_type_t type)
{
    ...
    case G920_FRAME_INPUT:
        return 4;
    /*
     * ⚠ Силы **через эту очередь не ходят**, и число 5 сегодня не читает
     * никто.
     *
     * С 03.08.2026 донгл шлёт их `g920_link_send`, то есть прямо в эфир,
     * минуя обе дисциплины: опоздавшая сила хуже пропущенной, а очередь —
     * это про порядок и повторы. Место в таблице оставлено на случай, если
     * силы когда-нибудь в очередь вернутся, но тогда **число придётся
     * пересмотреть**: сейчас оно ставит их ниже отчётов ввода, и это
     * решение никем не измерено — оно досталось от порядка объявления
     * типов, а не от замера. Замечание судьи, 03.08.2026.
     */
    case G920_FRAME_FFB:
        return 5;
    case G920_FRAME_DISCOVER:
