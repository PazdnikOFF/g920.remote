                          ...
                          const uint8_t *peer_mac, g920_rx_verdict_t verdict)
{
    ...
    if (frame->type == G920_FRAME_AUTH || frame->type == G920_FRAME_INPUT) {
        ...
        auth_alive = true;
        /*
         * Последний отчёт запоминается — до этой правки не запоминался
         * нигде.
         *
         * Вердикт судьи 03.08.2026: `last_input` и `last_input_len` были
         * объявлены и **не присваивались ни в одном месте**, поэтому
         * условие `last_input_len > 0` в главном цикле было ложно всегда, и
         * повтор раз в 25 мс не исполнился ни разу. Механизм существовал
         * только в документе.
         *
         * Здесь он оживлён, но **немедленная отдача оставлена**: она
         * работает и проверена человеком за рулём. Повтор закрывает ровно
         * ту дыру, ради которой писался, — заминку в эфире, когда консоль
         * иначе не получает **ничего** вместо последнего известного
         * состояния.
         *
         * ⚠ Строгую форму И6 («отдавать только по своему таймеру») это не
         * закрывает: джиттер радио по-прежнему протекает в тайминг USB.
...
void app_main(void)
{
    ...
    for (;;) {
        ...
        if (!have_identity && since_ask_ms >= IDENTITY_ASK_EVERY_MS) {
            ...
            since_ask_ms = 0;
            (void)g920_link_send(G920_FRAME_CONTROL, 0, 0, &code, sizeof(code));
        }
        ...
        if (g920_gip_device_configured() && !was_configured) {
            ...
            (void)send_hello(&sequence);
            {
                ...
                (void)g920_link_send(G920_FRAME_CONTROL, 0, 0, &code,
                                     sizeof(code));
            }
