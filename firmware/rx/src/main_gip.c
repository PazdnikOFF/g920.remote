 ...
 */
/* Просьба из колбэка линка перезапустить сессию с консолью. */
static volatile bool restart_session_wanted;
static volatile bool wheel_ready;
/* Сколько раз передатчик сообщил о готовности руля — видно в отчёте. */
static volatile uint32_t wheel_ready_seen;
...
/* Сколько отчётов ввода уже напечатано целиком — первых довольно. */
static uint8_t input_dumped;
 ...
 * принимает security, шлёт Start — и молчит минуту, игнорируя ввод.
 * Двадцать секунд с запасом: исправная консоль в тишине лежит десятками
 * секунд, но не минутами.
 */
                          ...
                          const uint8_t *peer_mac, g920_rx_verdict_t verdict)
{
    ...
    /*
     * Управляющий кадр от передатчика. Единственный, который он шлёт нам, —
     * «руль поднялся и стоит в Arrival». Это разрешение звать консоль:
     * знакомство должно начинаться со свежим рулём, иначе консоль проведёт
     * security с прошлым его экземпляром и потом молча игнорирует ввод.
     */
    if (frame->type == G920_FRAME_CONTROL && frame->length >= 1
        && frame->payload[0] == CONTROL_WHEEL_READY) {
        wheel_ready = true;
        wheel_ready_seen++;
        /*
         * Руль поднялся заново, а консоль держит принятой сессию его
         * прошлого экземпляра — эта сессия протухла, и знакомство надо
         * начинать заново **сразу**.
         *
         * Без этого связка выкарабкивается минутами: замерено на живом
         * стенде после снятия питания с руля — консоль успела прислать 500
         * запросов начальных отчётов (`0a=500`), прежде чем что-то
         * сдвинулось. Раньше этот случай ловился только приходом личности,
         * то есть **лишь когда передатчик перезагружался целиком**; если он
         * просто переподнял руль по USB, донгл об этом не узнавал вовсе.
         *
         * Сама перезагрузка сессии делается в главном цикле: здесь колбэк
         * линка, спать в нём нельзя.
         */
        if (auth_done) {
            restart_session_wanted = true;
        }
        return;
    }
    /*
    ...
    if (frame->type == G920_FRAME_AUTH || frame->type == G920_FRAME_INPUT) {
        ...
        auth_alive = true;
        /*
         * ⚠ Здесь **нельзя** снимать ожидание руля, и это стоило вечера.
         *
         * Раньше стояло `wheel_ready = true` на любой кадр из радио. Но
         * пока передатчик перезагружается, кадры продолжают идти от
         * **старого** руля — того самого, из чужой сессии, ради выхода из
         * которой всё и затевалось. Ожидание снималось мгновенно, донгл
         * звал консоль, и та начинала security с прошлым экземпляром руля.
         * Снаружи это «после включения бокса не работает ничего, кроме
         * кнопки Xbox».
         *
         * Признак готовности теперь приходит **явно**, отдельным сообщением
         * `CONTROL_WHEEL_READY`, которое передатчик шлёт по Announce руля.
         */
        /*
        ...
        if (frame->type == G920_FRAME_INPUT) {
            ...
            if (frame->length > 0 && frame->length <= sizeof(last_input)) {
                 ...
                 * я рассуждал о содержимом потока, ни разу на него не
                 * посмотрев: `0x20` из журнала исключён намеренно (он забил
                 * бы всё), и это молчание стоило нескольких неверных версий.
                 * Спека руля (`H001861`, Wheel State Report) задаёт байт 1 =
                 * Flags = 0x00 и байт 3 = длина >= 0x11 — есть с чем сверить.
                 */
                if (input_dumped < 3u) {
                    input_dumped++;
...
void app_main(void)
{
    ...
    for (;;) {
        ...
        /*
         * Руль вернулся под протухшей сессией — начинаем разговор с
         * консолью заново. Просьбу ставит колбэк линка, исполняем здесь:
         * переподключение спит внутри.
         */
        if (restart_session_wanted) {
            restart_session_wanted = false;
            if (g920_gip_device_configured()
                && !g920_gip_device_suspended()) {
                auth_done = false;
                host_answered = false;
                g920_gip_device_restart_session();
        ...
        /*
         * Консоль замолчала при состоявшейся сессии — начинаем разговор
         * заново.
         *
         * ⚠ Это **восстановление, а не лечение причины**. Почему консоль
         * после цикла питания принимает security, шлёт Start, а потом
         * замолкает и игнорирует ввод — я не выяснил. Но выход из этого
         * состояния воспроизводим: сброс донгла возвращает всё в строй,
         * проверено за вечер много раз. Раз человек делает это руками —
         * пусть делает донгл.
         *
         * Условия узкие, и каждое нужно:
         *
         * `auth_done` и `start_seen` — сессия действительно состоялась, то
         * есть это не «консоль ещё не дошла до нас», а «дошла и умолкла».
         *
         * `SESSION_STALE_MS` — с большим запасом над наблюдавшимся: рабочая
         * консоль в тишине лежит десятками секунд, и дёргать её раньше
         * значит рвать исправную сессию.
         *
         * Счётчик попыток — предохранитель от петли. В дежурном режиме
         * консоль тоже молчит неограниченно, и без ограничения донгл дёргал
         * бы шину вечно. Две попытки, дальше молчим до первого слова хоста:
         * оно обнуляет и счётчик, и таймер.
         */
        /*
         * ⚠ Здесь стоял перезапуск сессии по молчанию консоли, и он **снят**.
         *
         * Он работал: связка чинилась сама. Но чинилась ценой повторного
         * переподнятия руля, то есть лишнего калибровочного проворота каждые
         * одну-две минуты, — а человеку нужна **одна** калибровка при
         * включении и работающий руль сразу.
         *
         * Хуже того, он не мог не срабатывать вхолостую: исправная консоль в
         * тишине лежит по пятнадцать секунд и дольше, отличить её молчание
         * от больного этим признаком нельзя в принципе.
         *
         * Лечить надо порядок подъёма, а не последствия: консоль нельзя
         * звать, пока не вернулся руль. Этим занимается `wheel_ready`, и
         * теперь он ждёт явного слова передатчика, а не любого кадра.
         */
