...
static void take_identity(void)
{
    ...
    if (installed_len == length
        ...
        have_identity = true;
        /*
         * Личность приходит заново — значит **передатчик перезагрузился**,
         * а с ним заново поднялся и руль. Другого повода у него нет.
         *
         * Дальше всё решает вопрос: успела ли консоль принять security с
         * прошлым экземпляром руля.
         *
         * Успела — её сессия теперь **протухла**, и это не теория, а
         * замер: руль отдавал ввод (707 отчётов на нажатия, 992 доехало до
         * консоли, потерь ноль), security в новой сессии руля не было ни
         * одной, и консоль весь этот ввод молча игнорировала. Снаружи —
         * «работает только кнопка Xbox»: она отдельный тип сообщения и к
         * сессии руля не привязана. Лечится только тем, что консоль
         * начинает знакомство заново, то есть переподключением.
         *
         * Не успела (обычная загрузка) — трогать шину незачем и **нельзя**:
         * переподключение на ровном месте убивает живую сессию консоли и
         * будит выключенный бокс. Обе цены заплачены сегодня.
         *
         * Условия про `configured` и `suspended` обязательны: дёргать шину,
         * пока консоль спит или отсутствует, — это и есть то самое
         * самопроизвольное включение бокса.
         */
        if (auth_done && g920_gip_device_configured()
            && !g920_gip_device_suspended()) {
            auth_done = false;
            host_answered = false;
