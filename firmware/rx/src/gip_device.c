...
void tud_mount_cb(void)
{
    ...
    /* Хост настроил устройство — он с ним разговаривает, а не спит. Тот же
     * довод, что и в `gip_reset`, и та же цена ошибки. */
...
static void gip_reset(uint8_t rhport)
{
    ...
    s_gip.configured = false;
    /*
     * ⚠ **Сброс шины снимает и усыпление.** Устройство не может быть
     * усыплено, пока его сбрасывают: сброс — это активность на шине, то
     * есть прямая противоположность условию, по которому мы засыпали.
     *
     * Без этой строки флаг переживал уход консоли и её возвращение.
     * Замерено 03.08.2026 на цикле питания бокса: консоль вернулась,
     * прошла энумерацию, поставила конфигурацию — `configured` снова да, —
     * а `suspended` так и остался поднятым с прошлого раза. Он стоит
     * **первым барьером** в отправке, поэтому в новой сессии не проходило
     * ни одно сообщение: `sent` замерло на 14148 навсегда, отказы копились
     * по 192 за пять секунд, консоль ждала ввода, не дожидалась и бросала
     * устройство. Снаружи это выглядело как «после выключения и включения
     * бокса руль не работает».
     *
     * `tud_resume_cb` этот случай не закрывает: после сброса хост не
     * обязан присылать resume, он начинает разговор заново.
     */
    s_gip.suspended = false;
}
...
/*
 * Начать разговор с консолью заново: отключиться и подключиться.
 *
 * Отдельная функция, а не повторный вызов установки личности, потому что
 * повод другой и цена ошибки высокая. Переподключение — это для консоли
 * «воткнули геймпад», и делать его без нужды нельзя: именно так донгл
 * однажды сам включал выключенный бокс.
 *
 * Законный повод ровно один: **перезапустился руль**, а консоль держит
 * принятой сессию его прошлого экземпляра. Замерено 03.08.2026 — после
 * сброса передатчика руль отдавал ввод нормально (707 отчётов на нажатия,
 * 992 доехало до консоли), но security в новой сессии руля не было ни
 * одной (`from wheel: 06=0`), и консоль этот ввод молча игнорировала.
 * Снаружи: «работает только кнопка Xbox» — она отдельный тип сообщения и
 * сессии руля не касается.
 *
