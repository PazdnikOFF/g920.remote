...
/* Есть ли уже снятая личность в NVS: по ней решается, знакомиться с рулём
 * самим или сразу становиться трубой. */
static bool identity_in_nvs(void)
{
    uint8_t total[4];
    size_t got = 0;

    return g920_store_read("idlen", G920_STORE_KIND_IDENTITY, 1, total,
