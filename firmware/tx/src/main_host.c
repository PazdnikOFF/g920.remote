 ...
 *
 * ⚠ **Сколько ждём — не выбрано, а унаследовано.** Пир считается потерянным
 * после `G920_LINK_SILENCE_MS` (2 с), и это число заведено для пиринга:
 * «две секунды тишины — повод позвать снова». Инвариант И3 требует снимать
 * силы **немедленно**, и две секунды живого момента на рулевом моторе к
 * «немедленно» отношения не имеют. Замечание судьи 03.08.2026: число надо
 * вывести из замера — сколько линк молчит в худшем нормальном случае, — и
 * завести отдельно от таймаута пиринга. Сокращать его вслепую нельзя:
 * ложное срабатывание глушит обратную связь до следующего Loop State,
 * которого консоль не пришлёт.
 */
#define FFB_LOOP_FORCES_OFF 0x02u
#define FFB_LOOP_NORMAL 0x00u

static bool s_ffb_seen;
static bool s_had_peer;
...
void app_main(void)
{
    ...
    for (;;) {
         ...
         */
        if (s_claimed && s_ffb_seen) {
            bool peer = g920_link_has_peer();

            if (s_had_peer && !peer) {
                send_loop_state(FFB_LOOP_FORCES_OFF, "dead man: forces off");
                G920_LOGW(M1, "link lost while forces were live — forces off");
            } else if (!s_had_peer && peer) {
                send_loop_state(FFB_LOOP_NORMAL, "dead man: forces back");
                G920_LOGW(M1, "link back — force loop returned to normal");
            }
            s_had_peer = peer;
        } else {
            s_had_peer = g920_link_has_peer();
        }
